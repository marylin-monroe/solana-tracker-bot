...
export class WebhookServer {
  ...
  constructor(
    ...
    this.logger.info('🔥 WebhookServer: ИСПРАВЛЕНА логика buy/sell определения');
  }
 ...
 private async extractSwapInfoFromBalances(txData: SolanaWebhookPayload): Promise<{
    ...
    try {
      ...
      if (spentPaymentToken) {
        ...
        if (receivedNonPaymentToken) {
          ...
          console.log(`🟢 [WebhookServer] BUY DETECTED: ${this.tokenMetadataService.getTokenSymbol(inputMint)} → ${this.tokenMetadataService.getTokenSymbol(outputMint)}`);
        }
      } else {
        ...
        if (receivedPaymentToken) {
          ...
          if (spentNonPaymentToken) {
            ...
            console.log(`🔴 [WebhookServer] SELL DETECTED: ${this.tokenMetadataService.getTokenSymbol(inputMint)} → ${this.tokenMetadataService.getTokenSymbol(outputMint)}`);
          }
        }
      }
      ...
      if (inputIsPayment && outputIsPayment) {
        console.log(`🚫 [WebhookServer] FILTERED: Technical operation (payment to payment)`);
        return null;
      }

      // Также фильтруем операции между двумя неплатежными токенами (маловероятно, но возможно)
      if (!inputIsPayment && !outputIsPayment) {
        console.log(`🚫 [WebhookServer] FILTERED: Altcoin to altcoin swap`);
        return null;
      }
  ...
  private async processSmartMoneySwap(
    ...
    try {
      ...
      console.log(`🔥 [WebhookServer] ЕДИНЫЙ ЦЕНТР RESULT: ${swapType.toUpperCase()} ${amountUSD.toFixed(2)}`);
